...
/**
 * 作者：huangjiang
 * 描述： 日志打印类，在Application需要初始化 KLog.init(true)
 *
 * @Params
 * @return
 */
        ...
        switch (type) {
            ...
            case JSON: {
                ...
                if (jsonContent.toString().length() > 3200) {
                    KLog.d(tag, "jsonContent.length = " + jsonContent.toString().length());
                    int chunkCount = jsonContent.toString().length() / 3200;
                    for (int i = 0; i <= chunkCount; i++) {
                        int max = 3200 * (i + 1);
                        if (max >= jsonContent.toString().length()) {

                            KLog.d(tag, jsonContent.toString().substring(3200 * i));

                        } else {

                            KLog.d(tag, jsonContent.toString().substring(3200 * i, max));
                    ...
                    KLog.d(tag, jsonContent.toString());
